...
#define EXAMPLE_ESP_WIFI_SSID      "vagrant"
...
#define RED_LED_NUM GPIO_NUM_12
...
void timer_buttons(TimerHandle_t th) {
    gpio_set_level(RED_LED_NUM, 0);
}
...
void handle_button(void *param) {
    gpio_set_level(RED_LED_NUM, 1);
    xTimerStartFromISR(timer_button, NULL) ;
}
...
void app_main(void) {
    ...
    timer_led = xTimerCreate("TimerLed", pdMS_TO_TICKS(10), pdFALSE, 0, timer_green_led);
    gpio_isr_handler_add(TILT_SENSOR_NUM, handle_tilt, NULL);
    
     //button
    c.mode = GPIO_MODE_INPUT;
    c.pin_bit_mask = BUTTON_PIN;
    c.intr_type = GPIO_INTR_POSEDGE;
    c.pull_down_en = c.pull_up_en = 0;
    gpio_config(&c);

    timer_button = xTimerCreate("TimerButton", pdMS_TO_TICKS(100), pdFALSE, 0, timer_buttons);
    gpio_isr_handler_add(BUTTON_NUM, handle_button, NULL);
